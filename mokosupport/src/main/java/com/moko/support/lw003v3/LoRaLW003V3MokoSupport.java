...
import com.moko.support.lw003v3.handler.MokoCharacteristicHandler;
...
public class LoRaLW003V3MokoSupport extends MokoBleLib {
    private HashMap<OrderCHAR, BluetoothGattCharacteristic> mCharacteristicMap;

    private static volatile LoRaLW003V3MokoSupport INSTANCE;

    private Context mContext;

    private MokoBleConfig mBleConfig;

    private LoRaLW003V3MokoSupport() {
        //no instance
    }

    public static LoRaLW003V3MokoSupport getInstance() {
        if (INSTANCE == null) {
            synchronized (LoRaLW003V3MokoSupport.class) {
                if (INSTANCE == null) {
                    INSTANCE = new LoRaLW003V3MokoSupport();
                }
            }
        }
        return INSTANCE;
