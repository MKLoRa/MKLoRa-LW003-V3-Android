# LW003V3 Android SDK Guide（English）
...
**`lw003v3.LoRaLW003V3MokoSupport`**
...
**`lw003v3.OrderTaskAssembler`**

We assemble read data and write data to `OrderTask`, send the task to the device
through `LoRaLW003V3MokoSupport `, and receive the resopnse.
...
First of all, you should initialize the LoRaLW003V3MokoSupport.We recommend putting it in Application.
...
in LoRaLW003V3MokoSupport `AndroidManifest.xml`
...
```
LoRaLW003V3MokoSupport.getInstance().connDevice(beaconXInfo.mac);
```
...
```
LoRaLW003V3MokoSupport.getInstance().sendOrder(OrderTaskAssembler.setPassword(password));
...
```
// read lora region
LoRaLW003V3MokoSupport.getInstance().sendOrder(derTaskAssembler.getLoraRegion());
...
// get result
@Subscribe(threadMode = ThreadMode.MAIN)
public void onOrderTaskResponseEvent(OrderTaskResponseEvent event) {
...
2.`EventBus` is used in the SDK and can be modified in `LoRaLW003V3MokoSupport` if you want to use
